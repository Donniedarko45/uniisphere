...
const createAnonymousChat = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const waitingUser = yield prisma_1.default.user.findFirst({
            where: {
        ...
        if (waitingUser) {
            ...
            const chat = yield prisma_1.default.anonymousChat.create({
                data: {
            ...
            socket_1.io.to(waitingUser.id).emit('chat-matched', { chatId: chat.id, isUser1: true }); // waitingUser is user1
            socket_1.io.to(userId).emit('chat-matched', { chatId: chat.id, isUser1: false }); // requesting user is user2
...
const sendAnonymousMessage = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const message = yield prisma_1.default.anonymousMessage.create({
            data: {
                chatId,
                content,
                senderId: userId,
        ...
        socket_1.io.to(recipientId).emit('anonymous-message', Object.assign({}, message));
