...
const rateLimitHandler = (req: Request, res: Response) => {
  ...
  const retryAfter = resetTime instanceof Date ? Math.round((resetTime.getTime() - Date.now()) / 1000) : 0;
...
export const createUserSpecificLimiter = (maxRequests: number, windowMs: number, message: string) => {
  return rateLimit({
    windowMs,
    max: maxRequests,
