...
import { NotificationType, NotificationPriority, TargetType } from '../types/notification.types';
...
export const notifyPostComment = async (postId: string, commentedByUserId: string, commentContent: string) => {
  try {
    const post = await prisma.post.findUnique({
      where: { id: postId },
      select: { userId: true, content: true, mediaUrl: true }
...
export const notifyPostShare = async (postId: string, sharedByUserId: string) => {
  try {
    const post = await prisma.post.findUnique({
      where: { id: postId },
      select: { userId: true, content: true, mediaUrl: true }
...
export const notifyConnectionRequest = async (fromUserId: string, toUserId: string) => {
  try {
    await notificationService.createNotification({
      userId: toUserId,
      actorId: fromUserId,
...
export const notifyConnectionAccepted = async (acceptedByUserId: string, requestorUserId: string) => {
  try {
    await notificationService.createNotification({
...
export const notifyNewMessage = async (fromUserId: string, toUserId: string, messageContent: string) => {
  try {
    await notificationService.createNotification({
      userId: toUserId,
      actorId: fromUserId,
...
// Story notifications
export const notifyStoryView = async (storyId: string, viewedByUserId: string) => {
  try {
...
export const notifyBlogLike = async (blogId: string, likedByUserId: string) => {
  try {
    const blog = await prisma.blogs.findUnique({
      where: { id: blogId },
      select: { authorId: true, title: true, titlePhoto: true }
    });
...
export const notifyHumanLibMatch = async (userId: string, matchedWithUserId: string) => {
  try {
    await notificationService.createNotification({
      userId,
...
export const notifyMultipleUsers = async (userIds: string[], type: NotificationType, actorId?: string, targetId?: string) => {
  try {
...
export const notifyProfileView = async (profileOwnerId: string, viewedByUserId: string) => {
  try {
    ...
    const connection = await prisma.connection.findFirst({
      where: {
        OR: [
          { userId1: profileOwnerId, userId2: viewedByUserId, status: 'accepted' },
          { userId1: viewedByUserId, userId2: profileOwnerId, status: 'accepted' }
        ]
      }
    });
